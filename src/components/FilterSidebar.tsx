  ...
  <label className="flex items-center gap-3 cursor-pointer group py-1">
    <div className="relative flex items-center shrink-0">
      ...
      <div
        className="
          ...
          bg-transparent
          peer-checked:bg-acid
          peer-checked:border-acid
    ...
    <span
      className={`
        ...
        ${checked
          ? 'text-bone'
          : 'text-stone group-hover:text-bone'
        }
      `}
    >
      {label}
    ...
    <span className="font-mono text-xs text-stone/50 tabular-nums">
      {count}
    </span>
  </label>
);
...
const FilterGroup = ({ title, children }: FilterGroupProps) => (
  <div className="mb-6">
    <div className="flex items-center gap-3 mb-3">
      <h3 className="font-mono text-xs uppercase tracking-widest text-rust shrink-0">
        {title}
      </h3>
...
const FilterSidebar = ({
  ...
  return (
    <div className="border-b lg:border-b-0 lg:border-r border-white/8 p-4">
      ...
      <FilterGroup title="Period">
        {(['Cretaceous', 'Jurassic', 'Triassic'] as Species['period'][]).map(
          p => (
            <FilterRow
              key={p}
              label={p}
      ...
      <FilterGroup title="Clade">
        {(['theropod', 'sauropod', 'ornithischian'] as Species['clade'][]).map(
          c => (
            <FilterRow
              key={c}
      ...
      <FilterGroup title="Diet">
        {(['carnivore', 'herbivore', 'omnivore'] as Species['diet'][]).map(
          d => (
            <FilterRow
              key={d}
